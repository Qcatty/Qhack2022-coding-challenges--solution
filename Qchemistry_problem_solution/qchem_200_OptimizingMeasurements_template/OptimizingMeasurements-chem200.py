...
def compression_ratio(obs_hamiltonian, final_solution):
    ...
    # QHACK
    n_fin_observables=0
    n_init_observables=0
    for operation in obs_hamiltonian:
        if type(operation)==list:
            for gate in operation:
                n_init_observables+=1
        else:
            n_init_observables+=1
    for operation in final_solution:
        if type(operation)==list:
            for gate in operation:
                n_fin_observables+=1
        else:
            n_fin_observables+=1
    return 1-n_fin_observables/n_init_observables
    # QHACK
