...
def is_particle_preserving(circuit, n):
    ...
    for state in basis_states(n):
        num_electrons_basis=0
        for value in state:
            if value==1:
                num_electrons_basis+=1
        num_electrons_circuit=0
        for i,basis_coef in enumerate(circuit(state)):
            if basis_coef!=0:
                num_electrons_circuit=0
                for qubit in basis_states(n)[i]:
                    if qubit==1:
                        num_electrons_circuit+=1
                if num_electrons_basis!=num_electrons_circuit:
                    return False
    return True
    # QHACK #
